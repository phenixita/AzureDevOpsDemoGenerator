...
namespace VstsDemoBuilder.Controllers
{
    ...
    public class ExtractorController : LegacyController
    {
        ...
        public JsonResult GetprojectList(string accname, string pat)
        {
            ...
            string sessionPat = Session["PAT"]?.ToString();
            if (string.IsNullOrEmpty(sessionPat))
            {
                return Json(new { errmsg = "Session expired", count = 0 });
            }
            Configuration config = new Configuration() { AccountName = accname, PersonalAccessToken = sessionPat, UriString = defaultHost + accname, VersionNumber = ProjectCreationVersion };
        ...
        public JsonResult GetProjectProperties(string accname, string project, string _credentials)
        {
            ...
            {
                string defaultHost = VstsDemoBuilder.Infrastructure.AppSettings.Get("DefaultHost");
                string ProjectPropertyVersion = VstsDemoBuilder.Infrastructure.AppSettings.Get("ProjectPropertyVersion");

                string sessionPat = Session["PAT"]?.ToString();
                if (string.IsNullOrEmpty(sessionPat))
                {
                    return Json(new { errmsg = "Session expired" });
                }
                Configuration config = new Configuration() { AccountName = accname, PersonalAccessToken = sessionPat, UriString = defaultHost + accname, VersionNumber = ProjectPropertyVersion, ProjectId = project };
        ...
        public void EndEnvironmentSetupProcess(IAsyncResult result)
        {
            ...
            if (ExtractorService.StatusMessages.Keys.Count(x => x == strResult[0] + "_Errors") == 1)
            {
                ...
                if (errorMessages != "")
                {
                    ...
                    string logWIT = "true"; //VstsDemoBuilder.Infrastructure.AppSettings.Get("LogWIT");
                    if (logWIT == "true")
                    {
        ...
        public ProjectConfigurations ProjectConfiguration(Project model)
        {
            ...
            string defaultHost = VstsDemoBuilder.Infrastructure.AppSettings.Get("DefaultHost");
            string extensionHost = VstsDemoBuilder.Infrastructure.AppSettings.Get("ExtensionHost");
            string getReleaseVersion = VstsDemoBuilder.Infrastructure.AppSettings.Get("GetRelease");
